...
use App\Models\Procedure\PatientProcedureInformation;
...
class ProcedureController extends Controller
{
    ...
    public function patientProcedureList(Request $request){
        try {
            $token = $request->token;
            ...
            $patient_data = PatientProcedureInformation::select([
            ...
            if (empty($patient_data)) {
                ...
                return response()->json(['status' => 'Success', 'message' => 'Patient Procedure List data retrieved successfully', 'code' => 200, 'total_count' => count($patient_data), 'patient_list' => $patient_data]);
            }
        } catch (\Exception $e) {
            Log::debug($e->getMessage());
            return response()->json(['status' => 'error', 'code' => 500, 'message' => 'Please contact the administrator'], 500);
    ...
    public function patientProcedureDetails(Request $request){
        try {
            $token = $request->token;
            if (!$this->user_authentication($token)) {
                return response()->json(['status' => 'error', 'code' => 401, 'message' => 'Unauthorized'], 401);
            }
            $patient_data = PatientProcedureInformation::with(['patient_requests'])->select('*', DB::raw('concat(first_name, " ", middle_name, " ", surname) as patient_name'))->where('id', $request->patient_id)->first();
            ...
            if (empty($patient_data)) {
                ...
                return response()->json(['status' => 'Success', 'message' => 'Patient procedure data retrieved successfully', 'code' => 200, 'patient' => $patient_data]);
            }
        } catch (\Exception $e) {
            Log::debug($e->getMessage());
            return response()->json(['status' => 'error', 'code' => 500, 'message' => 'Please contact the administrator'], 500);
    ...
    public function patientMediationIndex(Request $request)
    {
        {
            try {
                $token = $request->token;
                ...
                return response()->json(['status' => 'Success', 'message' => 'Patient Mediation data retrieved successfully', 'code'=>200, 'total_count' => count($patient_mediation), 'data' => $patient_mediation], 200);
            } catch (\Exception $e) {
                log::debug($e->getMessage());
                return response()->json(['status' => 'error', 'code' => 500, 'message' => 'Please contact the administrator'], 500);
            }
    ...
    public function chIndicationStore(Request $request){
        try {
            $token = $request->token;
            ...
            if (isset($data) && !empty($data)) {
                ...
                if (isset($data['indication_data']) && !empty($data['indication_data'])) {
                    foreach ($data['indication_data'] as $indication) {
                        if (isset($indication['diagnosis']) && !empty($indication['diagnosis']) && isset($indication['code']) && !empty($indication['code']) && isset($indication['date']) && !empty($indication['date'])) {
                            $ch_indication = PatientChIndication::create([
                                'token' => $data['token'],
                                'stage_id' => $stage_type->id,
                                'mrn_number' => $data['mrn_number'],
                                'patient_id' => $data['patient_id'],
    ...
    public function chIndicationDelete(Request $request){
        try {
            $token = $request->token;
            ...
            if (isset($data['id']) && !empty($data['id']) && isset($data['mrn_number']) && !empty($data['mrn_number']) && isset($data['patient_id']) && !empty($data['patient_id'])) {
                ...
                if (isset($ch_indication) && !empty($ch_indication)) {
                    ...
                    $ch_indication->deleted_at = Carbon::now();
                    $ch_indication->save();
                }else{
                    return response()->json(['status' => 'error', 'code' => 204, 'message' => 'No data found'], 204);
                }
    ...
    public function patientLabDelete(Request $request){
        try {
            $token = $request->token;
            ...
            if (isset($data['id']) && !empty($data['id']) && isset($data['mrn_number']) && !empty($data['mrn_number']) && isset($data['patient_id']) && !empty($data['patient_id'])) {
                $patient_lab_data = PatientLab::where('id', $data['id'])
                    ->where('mrn_number', $data['mrn_number'])
                    ->where('patient_id', $data['patient_id'])
                    ->first();
                if (isset($patient_lab_data) && !empty($patient_lab_data)) {
                    $patient_lab_data->deleted_by = $data['deleted_by'];
                    $patient_lab_data->deleted_at = Carbon::now();
                    $patient_lab_data->save();
                }else{
                    return response()->json(['status' => 'error', 'code' => 204, 'message' => 'No data found'], 204);
                }
            } else {
    ...
    public function patientList(Request $request){
        try {
            ...
            $patient_data = PatientsInformation::orderBy('id', 'desc')->get();
    ...
    public function patientVitalsStore(Request $request)
    {
        try {
            DB::beginTransaction();
            ...
            if (isset($data['patient_id']) && !empty($data['patient_id'])) {
                $patient_vitals = PatientVitals::create($data);
                DB::commit();
                return response()->json(['status' => 'success', 'message' => 'Patient Vitals data saved successfully', 'code' => 200, 'data' => $patient_vitals], 200);
            } else {
                DB::rollBack();
